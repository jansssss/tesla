...
import { getAllGuides, getAllStaticGuides, getGuideBySlug, isStaticGuideBlinded } from "@/lib/guides";
...
export async function generateStaticParams() {
  // 블라인드여도 정적 페이지는 빌드한다(noindex 처리, 404 방지·되돌리기 용이).
  const staticSlugs = getAllStaticGuides().map((guide) => ({ slug: guide.slug }));
...
export async function generateMetadata({ params }) {
  ...
  // AdSense "가치가 낮은 콘텐츠" 대응으로 색인에서 제외하는 두 경우:
  //  (1) 정적 guides.js 글 — 전부 파이프라인 자동생성 → 전면 블라인드(noindex)
  //  (2) Supabase 자동 생성 글(content_html 없음)
  // 직접 작성한 Supabase 글(content_html 존재)만 색인 유지.
  const isBlindedStatic = isStaticGuideBlinded(slug);
  const isAutoGenerated = !staticGuide && !guide.contentHtml;
  const noindex = isBlindedStatic || isAutoGenerated;
...
export default async function GuideDetailPage({ params }) {
  ...
  // 관련글 풀: 블라인드된 정적 글끼리만 연결(noindex 페이지 내부 UX).
  // 직접 작성(Supabase 색인) 글에서는 블라인드 글로 링크가 새지 않도록 비노출.
  const allGuides = isStaticGuideBlinded(guide.slug) ? getAllStaticGuides() : getAllGuides();
