"""
paytesla.kr — GSC 주간 인사이트 리포트 생성기
...
Usage:
  ...
  python -m scripts.analytics.gsc_daily_insight --days 7 --site sc-domain:paytesla.kr
"""
...
# paytesla 는 "테슬라를 살까 말까 / 얼마나 드나"를 판단하러 오는 사이트다.
# 주제(보조금·충전·유지비)를 동사(계산·비교)보다 앞에 둔다 — "충전요금 얼마"는
# 계산 수요가 아니라 충전 비용 수요이고, 대응할 페이지도 충전 쪽이다.
_INTENT_RULES: list[tuple[str, tuple[str, ...]]] = [
    ("보조금", ("보조금", "지원금", "국고", "지자체", "보조 금액")),
    ("충전", ("충전", "슈퍼차저", "완속", "급속", "충전기", "전기요금", "충전요금", "집밥")),
    ("유지비", ("유지비", "보험", "세금", "자동차세", "취득세", "등록세", "취등록세",
                "정비", "타이어", "월납", "리스", "할부")),
    ("출고절차", ("출고", "계약", "인도", "대기", "신청", "방법", "절차", "서류", "어떻게")),
    ("자격조건", ("자격", "조건", "대상", "되나요", "가능한가", "제한", "요건", "법인")),
    ("비교", ("비교", "vs", "차이", "어느", "어디가", "유리", "낫나")),
    ("가격", ("가격", "price", "실구매가", "출고가", "인하", "인상", "할인", "프로모션")),
    ("계산", ("계산기", "계산", "얼마", "시뮬", "견적", "산정", "환산")),
    ("모델정보", ("모델3", "모델y", "model 3", "model y", "주행거리", "성능", "트림", "옵션")),
    ("사례후기", ("후기", "사례", "경험", "실제", "리뷰")),
    ("정의개념", ("뜻", "이란", "무엇", "의미", "개념")),
]

# 랜딩 페이지가 이 목록이면 "그 주제 전용 페이지가 없다" = 콘텐츠 공백 신호.
# /subsidy 는 대표 계산기(실제 상세 페이지)이므로 목록으로 보지 않는다.
_LISTING_PATHS = {
    "/", "/calc", "/guides", "/answers", "/models", "/compare", "/shop",
}

_PAGE_TYPE_PREFIXES = [
    ("/calc/", "calculator"),
    ("/subsidy/", "subsidy-region"),
    ("/answers/", "answer"),
    ("/guides/", "guide"),
    ("/models/", "model"),
    ("/compare/", "compare"),
...
def load_gsc_settings() -> dict:
    ...
    return {
        "site_url": os.getenv("GSC_SITE_URL") or "sc-domain:paytesla.kr",
    ...
    def _opportunities(self, now: dict, qp_rows: list[dict]) -> dict:
        ...
        for q, v in now.items():
            ...
            if imp >= min_imp and landing in _LISTING_PATHS:
                content_gap.append({
                    **base,
                    "action_hint": "전용 상세 페이지(계산기/답변) 신설 또는 기존 페이지 타겟팅 명확화 후보",
                })
            ...
            if p.get("clicks", 0) >= 2 and clicks < p["clicks"]:
                decaying.append({
                    **base,
                    ...
                    "action_hint": "콘텐츠 신선도 갱신(보조금 최신 수치·기준일) 검토",
                })
...
def render_markdown(r: dict) -> str:
    ...
    add(f"# paytesla GSC 인사이트 — {r['generated_for']} 기준")
    add("")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="paytesla.kr GSC 인사이트 리포트")
    p.add_argument("--days", type=int, default=14, help="분석 구간 길이 (일, 기본 14)")
    p.add_argument("--out", default="reports/gsc", help="리포트 출력 디렉터리")
    p.add_argument("--site", default=None, help="GSC 속성 (기본: GSC_SITE_URL 또는 sc-domain:paytesla.kr)")
    p.add_argument(
        "--fallback-days",
        type=int,
        default=180,
